...
function extractMetadataFromSingleDecorator(decorator, diagnostics) {
  ...
  switch (expr.kind) {
    case ts12.SyntaxKind.Identifier:
      metadataProperties.push(ts12.factory.createPropertyAssignment("type", expr));
      break;
    case ts12.SyntaxKind.CallExpression:
      const call = expr;
      metadataProperties.push(ts12.factory.createPropertyAssignment("type", call.expression));
      if (call.arguments.length) {
        const args = [];
        for (const arg of call.arguments) {
          args.push(arg);
        }
        const argsArrayLiteral = ts12.factory.createArrayLiteralExpression(ts12.factory.createNodeArray(args, true));
        metadataProperties.push(ts12.factory.createPropertyAssignment("args", argsArrayLiteral));
      }
      break;
    default:
      diagnostics.push({
        ...
        messageText: `${ts12.SyntaxKind[decorator.kind]} not implemented in gathering decorator metadata.`,
        category: ts12.DiagnosticCategory.Error,
        code: 0
      });
      break;
  }
  return ts12.factory.createObjectLiteralExpression(metadataProperties);
}
function createCtorParametersClassProperty(diagnostics, entityNameToExpression, ctorParameters, isClosureCompilerEnabled) {
  ...
  for (const ctorParam of ctorParameters) {
    if (!ctorParam.type && ctorParam.decorators.length === 0) {
      params.push(ts12.factory.createNull());
      continue;
    }
    const paramType = ctorParam.type ? typeReferenceToExpression(entityNameToExpression, ctorParam.type) : void 0;
    const members = [
      ts12.factory.createPropertyAssignment("type", paramType || ts12.factory.createIdentifier("undefined"))
    ];
    const decorators = [];
    ...
    if (decorators.length) {
      members.push(ts12.factory.createPropertyAssignment("decorators", ts12.factory.createArrayLiteralExpression(decorators)));
    }
    params.push(ts12.factory.createObjectLiteralExpression(members));
  }
  const initializer = ts12.factory.createArrowFunction(void 0, void 0, [], void 0, ts12.factory.createToken(ts12.SyntaxKind.EqualsGreaterThanToken), ts12.factory.createArrayLiteralExpression(params, true));
  const ctorProp = ts12.factory.createPropertyDeclaration([ts12.factory.createToken(ts12.SyntaxKind.StaticKeyword)], "ctorParameters", void 0, void 0, initializer);
  if (isClosureCompilerEnabled) {
    ts12.setSyntheticLeadingComments(ctorProp, [
      {
        kind: ts12.SyntaxKind.MultiLineCommentTrivia,
        text: [
          `*`,
...
function typeReferenceToExpression(entityNameToExpression, node) {
  ...
  switch (kind) {
    case ts12.SyntaxKind.FunctionType:
    case ts12.SyntaxKind.ConstructorType:
      return ts12.factory.createIdentifier("Function");
    case ts12.SyntaxKind.ArrayType:
    case ts12.SyntaxKind.TupleType:
      return ts12.factory.createIdentifier("Array");
    case ts12.SyntaxKind.TypePredicate:
    case ts12.SyntaxKind.TrueKeyword:
    case ts12.SyntaxKind.FalseKeyword:
    case ts12.SyntaxKind.BooleanKeyword:
      return ts12.factory.createIdentifier("Boolean");
    case ts12.SyntaxKind.StringLiteral:
    case ts12.SyntaxKind.StringKeyword:
      return ts12.factory.createIdentifier("String");
    case ts12.SyntaxKind.ObjectKeyword:
      return ts12.factory.createIdentifier("Object");
    case ts12.SyntaxKind.NumberKeyword:
    case ts12.SyntaxKind.NumericLiteral:
      return ts12.factory.createIdentifier("Number");
    case ts12.SyntaxKind.TypeReference:
      const typeRef = node;
      return entityNameToExpression(typeRef.typeName);
    case ts12.SyntaxKind.UnionType:
      const childTypeNodes = node.types.filter((t) => !(ts12.isLiteralTypeNode(t) && t.literal.kind === ts12.SyntaxKind.NullKeyword));
...
function symbolIsRuntimeValue(typeChecker, symbol) {
  ...
  return (symbol.flags & ts12.SymbolFlags.Value & ts12.SymbolFlags.ConstEnumExcludes) !== 0;
}
function getDownlevelDecoratorsTransform(typeChecker, host, diagnostics, isCore, isClosureCompilerEnabled, shouldTransformClass) {
  function addJSDocTypeAnnotation(node, jsdocType) {
    ...
    ts12.setSyntheticLeadingComments(node, [
      {
        kind: ts12.SyntaxKind.MultiLineCommentTrivia,
        text: `* @type {${jsdocType}} `,
        pos: -1,
  ...
  function createPropDecoratorsClassProperty(diagnostics2, properties) {
    ...
    for (const [name, decorators] of properties.entries()) {
      entries.push(ts12.factory.createPropertyAssignment(name, ts12.factory.createArrayLiteralExpression(decorators.map((deco) => extractMetadataFromSingleDecorator(deco, diagnostics2)))));
    }
    const initializer = ts12.factory.createObjectLiteralExpression(entries, true);
    const prop = ts12.factory.createPropertyDeclaration([ts12.factory.createToken(ts12.SyntaxKind.StaticKeyword)], "propDecorators", void 0, void 0, initializer);
  ...
  return (context) => {
    ...
    function entityNameToExpression(name) {
      ...
      if (ts12.isQualifiedName(name)) {
        const containerExpr = entityNameToExpression(name.left);
        if (containerExpr === void 0) {
          return void 0;
        }
        return ts12.factory.createPropertyAccessExpression(containerExpr, name.right);
      }
      const decl = symbol.declarations[0];
      if (isAliasImportDeclaration(decl)) {
        referencedParameterTypes?.add(decl);
        if (decl.name !== void 0) {
          return ts12.setOriginalNode(ts12.factory.createIdentifier(decl.name.text), decl.name);
        }
      }
      return ts12.setOriginalNode(ts12.factory.createIdentifier(name.text), name);
    }
    function transformClassElement(element) {
      element = ts12.visitEachChild(element, decoratorDownlevelVisitor, context);
      ...
      if (!element.name || !ts12.isIdentifier(element.name)) {
        diagnostics.push({
      ...
      const elementModifiers = ts12.canHaveModifiers(element) ? ts12.getModifiers(element) : void 0;
      let modifiers;
      if (decoratorsToKeep.length || elementModifiers?.length) {
        modifiers = ts12.setTextRange(ts12.factory.createNodeArray([...decoratorsToKeep, ...elementModifiers || []]), element.modifiers);
      }
      return [element.name.text, cloneClassElementWithModifiers(element, modifiers), toLower];
    }
    function transformConstructor(ctor) {
      ctor = ts12.visitEachChild(ctor, decoratorDownlevelVisitor, context);
      const newParameters = [];
      ...
      for (const param of oldParameters) {
        ...
        let modifiers;
        const paramModifiers = ts12.getModifiers(param);
        if (decoratorsToKeep.length || paramModifiers?.length) {
          modifiers = [...decoratorsToKeep, ...paramModifiers || []];
        }
        const newParam = ts12.factory.updateParameterDeclaration(param, modifiers, param.dotDotDotToken, param.name, param.questionToken, param.type, param.initializer);
        newParameters.push(newParam);
      }
      const updated = ts12.factory.updateConstructorDeclaration(ctor, ts12.getModifiers(ctor), newParameters, ctor.body);
    ...
    function transformClassDeclaration(classDecl) {
      ...
      for (const member of classDecl.members) {
        ...
        newMembers.push(ts12.visitEachChild(member, decoratorDownlevelVisitor, context));
      }
      ...
      const members = ts12.setTextRange(ts12.factory.createNodeArray(newMembers, classDecl.members.hasTrailingComma), classDecl.members);
      return ts12.factory.updateClassDeclaration(classDecl, classDecl.modifiers, classDecl.name, classDecl.typeParameters, classDecl.heritageClauses, members);
    }
    function decoratorDownlevelVisitor(node) {
      if (ts12.isClassDeclaration(node) && (shouldTransformClass === void 0 || shouldTransformClass(node))) {
        return transformClassDeclaration(node);
      }
      return ts12.visitEachChild(node, decoratorDownlevelVisitor, context);
    }
    return (sf) => {
      return ts12.visitEachChild(sf, decoratorDownlevelVisitor, context);
    };
  };
}
function cloneClassElementWithModifiers(node, modifiers) {
  let clone;
  if (ts12.isMethodDeclaration(node)) {
    clone = ts12.factory.createMethodDeclaration(modifiers, node.asteriskToken, node.name, node.questionToken, node.typeParameters, node.parameters, node.type, node.body);
  } else if (ts12.isPropertyDeclaration(node)) {
    clone = ts12.factory.createPropertyDeclaration(modifiers, node.name, node.questionToken, node.type, node.initializer);
  } else if (ts12.isGetAccessor(node)) {
    clone = ts12.factory.createGetAccessorDeclaration(modifiers, node.name, node.parameters, node.type, node.body);
  } else if (ts12.isSetAccessor(node)) {
    clone = ts12.factory.createSetAccessorDeclaration(modifiers, node.name, node.parameters, node.body);
  } else {
    throw new Error(`Unsupported decorated member with kind ${ts12.SyntaxKind[node.kind]}`);
  }
  return ts12.setOriginalNode(clone, node);
}
...
function unwrapExpression(node) {
  while (ts13.isAsExpression(node) || ts13.isParenthesizedExpression(node)) {
...
function expandForwardRef(arg) {
  ...
  if (!ts13.isArrowFunction(arg) && !ts13.isFunctionExpression(arg)) {
    return null;
  }
  const body = arg.body;
  if (ts13.isBlock(body)) {
    ...
    if (!ts13.isReturnStatement(stmt) || stmt.expression === void 0) {
      return null;
    }
...
function tryUnwrapForwardRef(node, reflector) {
  ...
  if (!ts13.isCallExpression(node) || node.arguments.length !== 1) {
    return null;
  }
  const fn = ts13.isPropertyAccessExpression(node.expression) ? node.expression.name : node.expression;
  if (!ts13.isIdentifier(fn)) {
    return null;
  }
...
function isExpressionForwardReference(expr, context, contextSource) {
  if (isWrappedTsNodeExpr(expr)) {
    const node = ts13.getOriginalNode(expr.node);
...
var parensWrapperTransformerFactory = (context) => {
  const visitor = (node) => {
    const visited = ts13.visitEachChild(node, visitor, context);
    if (ts13.isArrowFunction(visited) || ts13.isFunctionExpression(visited)) {
      return ts13.factory.createParenthesizedExpression(visited);
    }
    return visited;
  };
  return (node) => ts13.visitEachChild(node, visitor, context);
};
function wrapFunctionExpressionsInParens(expression) {
  return ts13.transform(expression, [parensWrapperTransformerFactory]).transformed[0];
}
...
var StaticInterpreter = class {
  ...
  visitIdentifier(node, context) {
    ...
    if (decl === null) {
      if (ts16.identifierToKeywordKind(node) === ts16.SyntaxKind.UndefinedKeyword) {
        return void 0;
      } else {
  ...
  visitType(node, context) {
    ...
    } else if (ts16.isTypeOperatorNode(node) && node.operator === ts16.SyntaxKind.ReadonlyKeyword) {
      return this.visitType(node.type, context);
...
function isVariableDeclarationDeclared(node) {
  ...
  const modifiers = ts16.getModifiers(varStmt);
  return modifiers !== void 0 && modifiers.some((mod) => mod.kind === ts16.SyntaxKind.DeclareKeyword);
}
var EMPTY = {};
...
function getContainerNode(node) {
  ...
  while (currentNode !== void 0) {
    switch (currentNode.kind) {
      case ts17.SyntaxKind.ExpressionStatement:
      case ts17.SyntaxKind.VariableStatement:
      case ts17.SyntaxKind.ReturnStatement:
      case ts17.SyntaxKind.IfStatement:
      case ts17.SyntaxKind.SwitchStatement:
      case ts17.SyntaxKind.DoStatement:
      case ts17.SyntaxKind.WhileStatement:
      case ts17.SyntaxKind.ForStatement:
      case ts17.SyntaxKind.ForInStatement:
      case ts17.SyntaxKind.ForOfStatement:
      case ts17.SyntaxKind.ContinueStatement:
      case ts17.SyntaxKind.BreakStatement:
      case ts17.SyntaxKind.ThrowStatement:
      case ts17.SyntaxKind.ObjectBindingPattern:
      case ts17.SyntaxKind.ArrayBindingPattern:
        return currentNode;
    }
...
function createGenerateUniqueIdentifierHelper() {
  ...
  return (sourceFile, symbolName) => {
    ...
    return ts18.factory.createUniqueName(name, ts18.GeneratedIdentifierFlags.Optimistic);
  };
}
...
function createTsTransformForImportManager(manager, extraStatementsForFiles) {
  return (ctx) => {
    ...
    const visitStatement = (node) => {
      if (!ts19.isImportDeclaration(node)) {
        return node;
      }
      if (deletedImports.has(node)) {
        return void 0;
      }
      if (node.importClause === void 0 || !ts19.isImportClause(node.importClause)) {
        return node;
      }
    ...
    return (sourceFile) => {
      ...
      sourceFile = ts19.visitEachChild(sourceFile, visitStatement, ctx);
...
function attemptToReuseGeneratedImports(tracker, request) {
  ...
  return [potentialNamespaceImport, ts20.factory.createIdentifier(request.exportSymbolName)];
}
...
function attemptToReuseExistingSourceFileImports(tracker, sourceFile, request) {
  ...
  for (let i = sourceFile.statements.length - 1; i >= 0; i--) {
    ...
    if (statement.importClause.namedBindings) {
      ...
      if (ts21.isNamespaceImport(namedBindings)) {
        tracker.reusedAliasDeclarations.add(namedBindings);
        if (request.exportSymbolName === null) {
          return namedBindings.name;
        }
        return [namedBindings.name, ts21.factory.createIdentifier(request.exportSymbolName)];
      }
      if (ts21.isNamedImports(namedBindings) && request.exportSymbolName !== null) {
  ...
  const propertyName = ts21.factory.createIdentifier(request.exportSymbolName);
  const fileUniqueAlias = request.unsafeAliasOverride ? ts21.factory.createIdentifier(request.unsafeAliasOverride) : tracker.generateUniqueIdentifier(sourceFile, request.exportSymbolName);
...
var ImportManager = class {
  ...
  _generateNewImport(request) {
    ...
    if (request.exportSymbolName === null || forceGenerateNamespacesForNewImports) {
      ...
      const namespaceImport2 = ts22.factory.createNamespaceImport(this.config.generateUniqueIdentifier(sourceFile, namespaceImportName) ?? ts22.factory.createIdentifier(namespaceImportName));
      ...
      if (request.exportSymbolName !== null) {
        return [namespaceImport2.name, ts22.factory.createIdentifier(request.exportSymbolName)];
      }
    ...
    const exportSymbolName = ts22.factory.createIdentifier(request.exportSymbolName);
    ...
    if (request.unsafeAliasOverride) {
      needsAlias = true;
      specifierName = ts22.factory.createIdentifier(request.unsafeAliasOverride);
    ...
    namedImports.get(request.exportModuleSpecifier).push(ts22.factory.createImportSpecifier(false, needsAlias ? exportSymbolName : void 0, specifierName));
  ...
  finalize() {
    ...
    this.reuseSourceFileImportsTracker.updatedImports.forEach((expressions, importDecl) => {
      ...
      const newElements = namedBindings.elements.concat(expressions.map(({ propertyName, fileUniqueAlias }) => ts22.factory.createImportSpecifier(false, fileUniqueAlias !== null ? propertyName : void 0, fileUniqueAlias ?? propertyName))).filter((specifier) => this._canAddSpecifier(sourceFile, moduleName, specifier));
      affectedFiles.add(sourceFile.fileName);
      if (newElements.length === 0) {
        deletedImports.add(importDecl);
      } else {
        updatedImportsResult.set(namedBindings, ts22.factory.updateNamedImports(namedBindings, newElements));
      }
    });
    this.removedImports.forEach((removeMap, sourceFile) => {
      ...
      if (!allImports) {
        allImports = sourceFile.statements.filter(ts22.isImportDeclaration);
        importDeclarationsPerFile.set(sourceFile, allImports);
      }
      for (const node of allImports) {
        if (!node.importClause?.namedBindings || !ts22.isNamedImports(node.importClause.namedBindings) || this.reuseSourceFileImportsTracker.updatedImports.has(node) || deletedImports.has(node)) {
          continue;
        }
        ...
        if (newImports.length === 0) {
          ...
          affectedFiles.add(sourceFile.fileName);
          updatedImportsResult.set(namedBindings, ts22.factory.updateNamedImports(namedBindings, newImports));
        }
      }
    });
...
function createImportReference(asTypeReference, ref) {
  if (asTypeReference) {
    return Array.isArray(ref) ? ts22.factory.createQualifiedName(ref[0], ref[1]) : ref;
  } else {
    return Array.isArray(ref) ? ts22.factory.createPropertyAccessExpression(ref[0], ref[1]) : ref;
  }
}
...
function canEmitType(type, canEmit) {
  ...
  function visitNode(node) {
    if (ts23.isImportTypeNode(node)) {
      return INELIGIBLE;
    }
    if (ts23.isTypeReferenceNode(node) && !canEmitTypeReference(node)) {
      return INELIGIBLE;
    } else {
      return ts23.forEachChild(node, visitNode);
    }
  }
...
var TypeEmitter = class {
    ...
    const typeReferenceTransformer = (context) => {
      const visitNode = (node) => {
        ...
        if (ts23.isTypeReferenceNode(node)) {
          ...
          let clone;
          if (ts23.isStringLiteral(node)) {
            clone = ts23.factory.createStringLiteral(node.text);
          } else if (ts23.isNumericLiteral(node)) {
            clone = ts23.factory.createNumericLiteral(node.text);
          } else if (ts23.isBigIntLiteral(node)) {
            clone = ts23.factory.createBigIntLiteral(node.text);
          } else if (ts23.isNoSubstitutionTemplateLiteral(node)) {
            clone = ts23.factory.createNoSubstitutionTemplateLiteral(node.text, node.rawText);
          } else if (ts23.isRegularExpressionLiteral(node)) {
            clone = ts23.factory.createRegularExpressionLiteral(node.text);
          } else {
            throw new Error(`Unsupported literal kind ${ts23.SyntaxKind[node.kind]}`);
          }
          ts23.setTextRange(clone, { pos: -1, end: -1 });
          return clone;
        } else {
          return ts23.visitEachChild(node, visitNode, context);
        }
      };
      return (node) => ts23.visitNode(node, visitNode, ts23.isTypeNode);
    };
    return ts23.transform(type, [typeReferenceTransformer]).transformed[0];
  }
  emitTypeReference(type) {
    ...
    if (type.typeArguments !== void 0) {
      typeArguments = ts23.factory.createNodeArray(type.typeArguments.map((typeArg) => this.emitType(typeArg)));
    }
    return ts23.factory.updateTypeReferenceNode(type, translatedType.typeName, typeArguments);
  }
};
...
function tsNumericExpression(value) {
  if (value < 0) {
    const operand = ts24.factory.createNumericLiteral(Math.abs(value));
    return ts24.factory.createPrefixUnaryExpression(ts24.SyntaxKind.MinusToken, operand);
  }
  return ts24.factory.createNumericLiteral(value);
}
...
var TypeTranslatorVisitor = class {
  ...
  visitExpressionType(type, context) {
    ...
    if (!ts25.isTypeReferenceNode(typeNode)) {
      throw new Error("An ExpressionType with type arguments must translate into a TypeReferenceNode");
    } else if (typeNode.typeArguments !== void 0) {
      throw new Error(`An ExpressionType with type arguments cannot have multiple levels of type arguments`);
    }
    const typeArgs = type.typeParams.map((param) => this.translateType(param, context));
    return ts25.factory.createTypeReferenceNode(typeNode.typeName, typeArgs);
  }
  visitArrayType(type, context) {
    return ts25.factory.createArrayTypeNode(this.translateType(type.of, context));
  }
  visitMapType(type, context) {
    const parameter = ts25.factory.createParameterDeclaration(void 0, void 0, "key", void 0, ts25.factory.createKeywordTypeNode(ts25.SyntaxKind.StringKeyword));
    const typeArgs = type.valueType !== null ? this.translateType(type.valueType, context) : ts25.factory.createKeywordTypeNode(ts25.SyntaxKind.UnknownKeyword);
    const indexSignature = ts25.factory.createIndexSignature(void 0, [parameter], typeArgs);
    return ts25.factory.createTypeLiteralNode([indexSignature]);
  }
  visitTransplantedType(ast, context) {
    const node = ast.type instanceof Reference ? ast.type.node : ast.type;
    if (!ts25.isTypeNode(node)) {
  ...
  visitReadVarExpr(ast, context) {
    ...
    return ts25.factory.createTypeQueryNode(ts25.factory.createIdentifier(ast.name));
  }
  ...
  visitLiteralExpr(ast, context) {
    if (ast.value === null) {
      return ts25.factory.createLiteralTypeNode(ts25.factory.createNull());
    } else if (ast.value === void 0) {
      return ts25.factory.createKeywordTypeNode(ts25.SyntaxKind.UndefinedKeyword);
    } else if (typeof ast.value === "boolean") {
      return ts25.factory.createLiteralTypeNode(ast.value ? ts25.factory.createTrue() : ts25.factory.createFalse());
    } else if (typeof ast.value === "number") {
      return ts25.factory.createLiteralTypeNode(tsNumericExpression(ast.value));
    } else {
      return ts25.factory.createLiteralTypeNode(ts25.factory.createStringLiteral(ast.value));
    }
  }
  ...
  visitExternalExpr(ast, context) {
    ...
    return ts25.factory.createTypeReferenceNode(typeName, typeArguments);
  }
  ...
  visitLiteralArrayExpr(ast, context) {
    ...
    return ts25.factory.createTupleTypeNode(values);
  }
  visitLiteralMapExpr(ast, context) {
    const entries = ast.entries.map((entry) => {
      ...
      return ts25.factory.createPropertySignature(
        ...
        void 0,
        /* name */
        quoted ? ts25.factory.createStringLiteral(key) : key,
        /* questionToken */
        void 0,
        /* type */
        type
    ...
    return ts25.factory.createTypeLiteralNode(entries);
  }
  ...
  visitTypeofExpr(ast, context) {
    ...
    if (!ts25.isTypeReferenceNode(typeNode)) {
      throw new Error(`The target of a typeof expression must be a type reference, but it was
          ${ts25.SyntaxKind[typeNode.kind]}`);
    }
    return ts25.factory.createTypeQueryNode(typeNode.typeName);
  }
  ...
  translateType(type, context) {
    ...
    if (!ts25.isTypeNode(typeNode)) {
      throw new Error(`A Type must translate to a TypeNode, but was ${ts25.SyntaxKind[typeNode.kind]}`);
    }
    return typeNode;
  }
  translateExpression(expr, context) {
    ...
    if (!ts25.isTypeNode(typeNode)) {
      throw new Error(`An Expression must translate to a TypeNode, but was ${ts25.SyntaxKind[typeNode.kind]}`);
    }
    return typeNode;
  }
  translateTypeReference(type, context, viaModule) {
    const target = ts25.isIdentifier(type.typeName) ? type.typeName : type.typeName.right;
    ...
    if (!ts25.isTypeReferenceNode(typeNode)) {
      throw new Error(`Expected TypeReferenceNode for emitted reference, got ${ts25.SyntaxKind[typeNode.kind]}.`);
    }
    return typeNode;
  }
};
...
var TypeScriptAstFactory = class {
  ...
  UNARY_OPERATORS = (() => ({
    ...
    "!": ts26.SyntaxKind.ExclamationToken
  }))();
  BINARY_OPERATORS = (() => ({
    ...
    "**": ts26.SyntaxKind.AsteriskAsteriskToken,
    ...
    "*=": ts26.SyntaxKind.AsteriskEqualsToken,
    "/=": ts26.SyntaxKind.SlashEqualsToken,
    "%=": ts26.SyntaxKind.PercentEqualsToken,
  ...
  VAR_TYPES = (() => ({
    ...
    "let": ts26.NodeFlags.Let,
    "var": ts26.NodeFlags.None
  }))();
  constructor(annotateForClosureCompiler) {
    this.annotateForClosureCompiler = annotateForClosureCompiler;
  }
  attachComments = attachComments;
  createArrayLiteral = ts26.factory.createArrayLiteralExpression;
  createAssignment(target, operator, value) {
    return ts26.factory.createBinaryExpression(target, this.BINARY_OPERATORS[operator], value);
  }
  createBinaryExpression(leftOperand, operator, rightOperand) {
    return ts26.factory.createBinaryExpression(leftOperand, this.BINARY_OPERATORS[operator], rightOperand);
  }
  createBlock(body) {
    return ts26.factory.createBlock(body);
  }
  createCallExpression(callee, args, pure) {
    const call = ts26.factory.createCallExpression(callee, void 0, args);
    if (pure) {
      ts26.addSyntheticLeadingComment(
        call,
        ts26.SyntaxKind.MultiLineCommentTrivia,
  ...
  createConditional(condition, whenTrue, whenFalse) {
    return ts26.factory.createConditionalExpression(condition, void 0, whenTrue, void 0, whenFalse);
  }
  createElementAccess = ts26.factory.createElementAccessExpression;
  createExpressionStatement = ts26.factory.createExpressionStatement;
  createDynamicImport(url) {
    return ts26.factory.createCallExpression(
      ts26.factory.createToken(ts26.SyntaxKind.ImportKeyword),
      /* type */
      void 0,
      [typeof url === "string" ? ts26.factory.createStringLiteral(url) : url]
    );
  }
  createFunctionDeclaration(functionName, parameters, body) {
    ...
    return ts26.factory.createFunctionDeclaration(void 0, void 0, functionName, void 0, parameters.map((param) => ts26.factory.createParameterDeclaration(void 0, void 0, param)), void 0, body);
  }
  createFunctionExpression(functionName, parameters, body) {
    if (!ts26.isBlock(body)) {
      throw new Error(`Invalid syntax, expected a block, but got ${ts26.SyntaxKind[body.kind]}.`);
    }
    return ts26.factory.createFunctionExpression(void 0, void 0, functionName ?? void 0, void 0, parameters.map((param) => ts26.factory.createParameterDeclaration(void 0, void 0, param)), void 0, body);
  }
  createArrowFunctionExpression(parameters, body) {
    if (ts26.isStatement(body) && !ts26.isBlock(body)) {
      throw new Error(`Invalid syntax, expected a block, but got ${ts26.SyntaxKind[body.kind]}.`);
    }
    return ts26.factory.createArrowFunction(void 0, void 0, parameters.map((param) => ts26.factory.createParameterDeclaration(void 0, void 0, param)), void 0, void 0, body);
  }
  createIdentifier = ts26.factory.createIdentifier;
  createIfStatement(condition, thenStatement, elseStatement) {
    return ts26.factory.createIfStatement(condition, thenStatement, elseStatement ?? void 0);
  }
  createLiteral(value) {
    if (value === void 0) {
      return ts26.factory.createIdentifier("undefined");
    } else if (value === null) {
      return ts26.factory.createNull();
    } else if (typeof value === "boolean") {
      return value ? ts26.factory.createTrue() : ts26.factory.createFalse();
    } else if (typeof value === "number") {
      return tsNumericExpression(value);
    } else {
      return ts26.factory.createStringLiteral(value);
    }
  }
  ...
  createTaggedTemplate(tag, template) {
    return ts26.factory.createTaggedTemplateExpression(tag, void 0, this.createTemplateLiteral(template));
  }
  createTemplateLiteral(template) {
    ...
    if (length === 1) {
      templateLiteral = ts26.factory.createNoSubstitutionTemplateLiteral(head.cooked, head.raw);
    } else {
      const spans = [];
      for (let i = 1; i < length - 1; i++) {
        ...
        spans.push(ts26.factory.createTemplateSpan(template.expressions[i - 1], middle));
      }
      ...
      spans.push(ts26.factory.createTemplateSpan(resolvedExpression, templateTail));
      templateLiteral = ts26.factory.createTemplateExpression(ts26.factory.createTemplateHead(head.cooked, head.raw), spans);
    }
    if (head.range !== null) {
  ...
  createThrowStatement = ts26.factory.createThrowStatement;
  createTypeOfExpression = ts26.factory.createTypeOfExpression;
  createVoidExpression = ts26.factory.createVoidExpression;
  ...
  setSourceMapRange(node, sourceMapRange) {
    ...
    if (!this.externalSourceFiles.has(url)) {
      this.externalSourceFiles.set(url, ts26.createSourceMapSource(url, sourceMapRange.content, (pos) => pos));
    }
    const source = this.externalSourceFiles.get(url);
    ts26.setSourceMapRange(node, {
...
function createTemplateMiddle(cooked, raw) {
  ...
  node.kind = ts26.SyntaxKind.TemplateMiddle;
  return node;
}
function createTemplateTail(cooked, raw) {
  const node = ts26.factory.createTemplateHead(cooked, raw);
  node.kind = ts26.SyntaxKind.TemplateTail;
  return node;
}
function attachComments(statement, leadingComments) {
  for (const comment of leadingComments) {
    const commentKind = comment.multiline ? ts26.SyntaxKind.MultiLineCommentTrivia : ts26.SyntaxKind.SingleLineCommentTrivia;
    if (comment.multiline) {
      ts26.addSyntheticLeadingComment(statement, commentKind, comment.toString(), comment.trailingNewline);
    } else {
      for (const line of comment.toString().split("\n")) {
        ts26.addSyntheticLeadingComment(statement, commentKind, line, comment.trailingNewline);
      }
    }
  }
}
  ...
  ctorParams.forEach((param, idx) => {
    ...
    (param.decorators || []).filter((dec) => isCore || isAngularCore(dec)).forEach((dec) => {
      ...
      if (name === "Inject") {
        ...
        if (ts33.isStringLiteralLike(attributeName)) {
          attributeNameType = new LiteralExpr(attributeName.text);
        } else {
          attributeNameType = new WrappedNodeExpr3(ts33.factory.createKeywordTypeNode(ts33.SyntaxKind.UnknownKeyword));
        }
      } else {
...
function resolveLiteral(decorator, literalCache) {
  ...
  if (!ts35.isObjectLiteralExpression(meta)) {
...
function extractClassMetadata(clazz, reflection, isCore, annotateForClosureCompiler, angularDecoratorTransform = (dec) => dec, undecoratedMetadataExtractor = () => null) {
  ...
  const metaDecorators = new WrappedNodeExpr4(ts36.factory.createArrayLiteralExpression(ngClassDecorators));
  ...
  for (const member of classMembers) {
    const shouldQuoteName = member.nameNode !== null && ts36.isStringLiteralLike(member.nameNode);
...
function ctorParameterToMetadata(param, isCore) {
  ...
  if (param.decorators !== null) {
    ...
    const value = new WrappedNodeExpr4(ts36.factory.createArrayLiteralExpression(ngDecorators));
    mapEntries.push({ key: "decorators", value, quoted: false });
  }
  return literalMap(mapEntries);
}
function decoratedClassMemberToMetadata(decorators, isCore) {
  const ngDecorators = decorators.filter((dec) => isAngularDecorator3(dec, isCore)).map((decorator) => new WrappedNodeExpr4(decoratorToMetadata(decorator)));
...
function decoratorToMetadata(decorator, wrapFunctionsInParens) {
  ...
  const properties = [
    ts36.factory.createPropertyAssignment("type", decorator.identifier)
  ];
  if (decorator.args !== null && decorator.args.length > 0) {
    const args = decorator.args.map((arg) => {
      return wrapFunctionsInParens ? wrapFunctionExpressionsInParens(arg) : arg;
    });
    properties.push(ts36.factory.createPropertyAssignment("args", ts36.factory.createArrayLiteralExpression(args)));
  }
  return ts36.factory.createObjectLiteralExpression(properties, true);
}
function isAngularDecorator3(decorator, isCore) {
  return isCore || decorator.import !== null && decorator.import.from === "@angular/core";
}
function removeIdentifierReferences(node, names) {
  const result = ts36.transform(node, [
    (context) => (root) => ts36.visitNode(root, function walk(current) {
      return ts36.isIdentifier(current) && (typeof names === "string" ? current.text === names : names.has(current.text)) ? ts36.factory.createIdentifier(current.text) : ts36.visitEachChild(current, walk, context);
    })
  ]);
...
function tryParseInitializerApi(functions, expression, reflector, importTracker) {
  if (ts37.isAsExpression(expression) || ts37.isParenthesizedExpression(expression)) {
    return tryParseInitializerApi(functions, expression.expression, reflector, importTracker);
  }
  if (!ts37.isCallExpression(expression)) {
    return null;
  }
...
function parseTopLevelRequiredCall(call, functions, importTracker) {
  const node = call.expression;
  if (!ts37.isPropertyAccessExpression(node) || !ts37.isIdentifier(node.expression) || node.name.text !== "required") {
    return null;
  }
...
function parseTopLevelCallFromNamespace(call, functions, importTracker) {
  ...
  if (ts37.isIdentifier(node.expression) && ts37.isIdentifier(node.name)) {
    ...
    ts37.isPropertyAccessExpression(node.expression) && ts37.isIdentifier(node.expression.expression) && ts37.isIdentifier(node.expression.name) && node.name.text === "required"
  ) {
...
function parseAndValidateInputAndOutputOptions(optionsNode) {
  if (!ts38.isObjectLiteralExpression(optionsNode)) {
  ...
  if (options.has("alias")) {
    ...
    if (!ts38.isStringLiteralLike(aliasExpr)) {
...
function tryParseSignalQueryFromInitializer(member, reflector, importTracker) {
  ...
  if (optionsNode !== void 0 && !ts39.isObjectLiteralExpression(optionsNode)) {
...
function parseLocator(expression, reflector) {
  ...
  if (ts39.isStringLiteralLike(expression)) {
...
function parseReadOption(value) {
  if (ts39.isExpressionWithTypeArguments(value) || ts39.isParenthesizedExpression(value) || ts39.isAsExpression(value)) {
    return parseReadOption(value.expression);
  }
  if (ts39.isPropertyAccessExpression(value) && ts39.isIdentifier(value.expression) || ts39.isIdentifier(value)) {
...
function parseDescendantsOption(value) {
  if (value.kind === ts39.SyntaxKind.TrueKeyword) {
    return true;
  } else if (value.kind === ts39.SyntaxKind.FalseKeyword) {
    return false;
  }
...
function extractDecoratorQueryMetadata(exprNode, name, args, propertyName, reflector, evaluator) {
  ...
  if (args.length === 2) {
    ...
    if (!ts40.isObjectLiteralExpression(optionsExpr)) {
...
function extractHostBindings(members, evaluator, coreModule, compilationMode, hostBindingNodes, metadata) {
  ...
  if (metadata && metadata.has("host")) {
    ...
    if (ts40.isObjectLiteralExpression(hostExpression)) {
  ...
  filterToMembersWithDecorator(members, "HostBinding", coreModule).forEach(({ member, decorators }) => {
    decorators.forEach((decorator) => {
      ...
      if (ts40.isDecorator(decorator.node)) {
...
function extractQueriesFromDecorator(queryData, reflector, evaluator, isCore) {
  const content = [];
  const view = [];
  if (!ts40.isObjectLiteralExpression(queryData)) {
  ...
  reflectObjectLiteral(queryData).forEach((queryExpr, propertyName) => {
    ...
    if (!ts40.isNewExpression(queryExpr)) {
    ...
    const queryType = ts40.isPropertyAccessExpression(queryExpr.expression) ? queryExpr.expression.name : queryExpr.expression;
    if (!ts40.isIdentifier(queryType)) {
      throw new FatalDiagnosticError(ErrorCode.VALUE_HAS_WRONG_TYPE, queryData, "Decorator query metadata must be an instance of a query type");
    }
  ...
  return (member) => {
    ...
    if (input !== null) {
      return getDecoratorMetaArray([
        [new ExternalExpr4(R3Identifiers.inputDecorator), memberMetadataFromSignalInput(input)]
      ]);
    }
    ...
    if (output !== null) {
      return getDecoratorMetaArray([
        [
          new ExternalExpr4(R3Identifiers.outputDecorator),
    ...
    if (model !== null) {
      return getDecoratorMetaArray([
        [
          new ExternalExpr4(R3Identifiers.inputDecorator),
    ...
    if (query !== null) {
      ...
      return getDecoratorMetaArray([
        [new ExternalExpr4(identifier), memberMetadataFromSignalQuery(query.call)]
      ]);
    }
    return null;
  };
...
function memberMetadataFromSignalQuery(call) {
  ...
  const firstArgMeta = ts40.isStringLiteralLike(firstArg) || ts40.isCallExpression(firstArg) ? new WrappedNodeExpr6(firstArg) : (
    ...
    new ExternalExpr4(R3Identifiers.forwardRef).callFn([
  ...
  const entries = [
    ...
    firstArgMeta,
    new WrappedNodeExpr6(ts40.factory.createObjectLiteralExpression([
      ...call.arguments.length > 1 ? [ts40.factory.createSpreadAssignment(call.arguments[1])] : [],
      ts40.factory.createPropertyAssignment("isSignal", ts40.factory.createTrue())
    ]))
  ];
...
function parseDecoratorInputTransformFunction(clazz, classPropertyName, value, reflector, refEmitter, compilationMode, emitDeclarationOnly) {
  ...
  if (compilationMode === CompilationMode.LOCAL) {
    ...
    return {
      node: node2,
      type: new Reference(ts40.factory.createKeywordTypeNode(ts40.SyntaxKind.UnknownKeyword))
    };
  }
...
function assertEmittableInputType(type, contextFile, reflector, refEmitter) {
  (function walk(node) {
    if (ts40.isTypeReferenceNode(node) && ts40.isIdentifier(node.typeName)) {
...
function getHostBindingErrorNode(error, hostExpr) {
  if (ts40.isObjectLiteralExpression(hostExpr)) {
    for (const prop of hostExpr.properties) {
      if (ts40.isPropertyAssignment(prop) && ts40.isStringLiteralLike(prop.initializer) && error.msg.includes(`[${prop.initializer.text}]`)) {
...
function extractHostDirectives(rawHostDirectives, evaluator, reflector, compilationMode, forwardRefResolver, emitDeclarationOnly) {
  ...
  return resolved.map((value) => {
    ...
    if (compilationMode === CompilationMode.LOCAL && hostReference instanceof DynamicValue) {
      if (!ts40.isIdentifier(hostReference.node) && !ts40.isPropertyAccessExpression(hostReference.node)) {
      ...
      if (emitDeclarationOnly) {
        if (ts40.isIdentifier(hostReference.node)) {
...
var ShimAdapter = class {
  ...
  maybeGenerate(fileName) {
    ...
    for (const record of this.generators) {
      ...
      let inputFile = this.delegate.getSourceFile(baseFileName, ts41.ScriptTarget.Latest);
      if (inputFile === void 0) {
        baseFileName = absoluteFrom(prefix + ".tsx");
        inputFile = this.delegate.getSourceFile(baseFileName, ts41.ScriptTarget.Latest);
      }
...
function extractTemplate(node, template, evaluator, depTracker, resourceLoader, options, compilationMode) {
  if (template.isInline) {
    ...
    let sourceMapUrl;
    if (ts42.isStringLiteral(template.expression) || ts42.isNoSubstitutionTemplateLiteral(template.expression)) {
...
function getTemplateRange(templateExpr) {
  ...
  const { line, character } = ts42.getLineAndCharacterOfPosition(templateExpr.getSourceFile(), startPos);
  return {
    startPos,
    startLine: line,
    startCol: character,
...
function transformDecoratorResources(dec, component, styles, template) {
  ...
  if (metadata.has("templateUrl")) {
    ...
    metadata.set("template", ts42.factory.createStringLiteral(template.content));
  }
    ...
    if (styles.length > 0) {
      const styleNodes = styles.reduce((result, style) => {
        if (style.trim().length > 0) {
          result.push(ts42.factory.createStringLiteral(style));
        }
        return result;
      }, []);
      if (styleNodes.length > 0) {
        metadata.set("styles", ts42.factory.createArrayLiteralExpression(styleNodes));
      }
    }
  }
  const newMetadataFields = [];
  for (const [name, value] of metadata.entries()) {
    newMetadataFields.push(ts42.factory.createPropertyAssignment(name, value));
  }
  return { ...dec, args: [ts42.factory.createObjectLiteralExpression(newMetadataFields)] };
}
...
function extractStyleUrlsFromExpression(evaluator, styleUrlsExpr) {
  const styleUrls = [];
  if (ts42.isArrayLiteralExpression(styleUrlsExpr)) {
    for (const styleUrlExpr of styleUrlsExpr.elements) {
      if (ts42.isSpreadElement(styleUrlExpr)) {
...
function extractInlineStyleResources(component) {
  ...
  function stringLiteralElements(array) {
    return array.elements.filter((e) => ts42.isStringLiteralLike(e));
  }
  const stylesExpr = component.get("styles");
  if (stylesExpr !== void 0) {
    if (ts42.isArrayLiteralExpression(stylesExpr)) {
      for (const expression of stringLiteralElements(stylesExpr)) {
        styles.add({ path: null, node: expression });
      }
    } else if (ts42.isStringLiteralLike(stylesExpr)) {
...
function getSymbolIdentifier(decl) {
  if (!ts43.isSourceFile(decl.parent)) {
    return null;
  }
...
function extractSemanticTypeParameters(node) {
  if (!ts44.isClassDeclaration(node) || node.typeParameters === void 0) {
    return null;
  }
...
import { ExternalExpr as ExternalExpr6 } from "@angular/compiler";
...
var LocalModuleScopeRegistry = class {
  ...
  getExportedScope(ref, diagnostics, ownerForErrors, type) {
    if (ref.node.getSourceFile().isDeclarationFile) {
      if (!ts45.isClassDeclaration(ref.node)) {
  ...
  getReexports(ngModule, ref, declared, exported, diagnostics) {
    ...
    const addReexport = (exportRef) => {
      ...
      if (!reexportMap.has(exportName)) {
        if (exportRef.alias && exportRef.alias instanceof ExternalExpr6) {
          reexports.push({
          ...
          if (!(expr instanceof ExternalExpr6) || expr.value.moduleName === null || expr.value.name === null) {
...
var SelectorlessComponentScopeReader = class {
  ...
  getAvailableIdentifiers(node) {
    const result = /* @__PURE__ */ new Map();
    let current = ts46.getOriginalNode(node).parent;
    while (current) {
      if (!ts46.isSourceFile(current) && !ts46.isBlock(current)) {
...
function isSymbolAliasOf(firstSymbol, lastSymbol, typeChecker) {
  ...
  while (firstSymbol !== currentSymbol && currentSymbol !== void 0 && currentSymbol.flags & ts48.SymbolFlags.Alias) {
...
var TsCreateProgramDriver = class {
  ...
  updateFiles(contents, updateMode) {
    ...
    for (const [filePath, { newText, originalFile }] of contents.entries()) {
      const sf = ts49.createSourceFile(filePath, newText, ts49.ScriptTarget.Latest, true);
    ...
    this.program = ts49.createProgram({
      host,
...
function makeTemplateDiagnostic(id, mapping, span, category, code, messageText, relatedMessages, deprecatedDiagInfo) {
  if (mapping.type === "direct") {
    ...
    if (relatedMessages !== void 0) {
      for (const relatedMessage of relatedMessages) {
        relatedInformation.push({
          category: ts50.DiagnosticCategory.Message,
          code: 0,
    ...
    let typeForMessage;
    if (category === ts50.DiagnosticCategory.Warning) {
      typeForMessage = "Warning";
    } else if (category === ts50.DiagnosticCategory.Suggestion) {
      typeForMessage = "Suggestion";
    } else if (category === ts50.DiagnosticCategory.Message) {
...
function parseTemplateAsSourceFile(fileName, template) {
  ...
  return ts50.createSourceFile(
    fileName,
    template,
    ts50.ScriptTarget.Latest,
    /* setParentNodes */
    false,
    ts50.ScriptKind.JSX
  );
...
var CompletionEngine = class {
  ...
  getExpressionCompletionLocation(expr) {
    ...
    if (expr instanceof PropertyRead) {
      ...
      const ternaryExpr = findFirstMatchingNode(this.tcb, {
        filter: ts51.isParenthesizedExpression,
        withSpan: expr.sourceSpan
      });
      if (ternaryExpr === null || !ts51.isConditionalExpression(ternaryExpr.expression)) {
        return null;
      }
      const whenTrue = ternaryExpr.expression.whenTrue;
      if (ts51.isPropertyAccessExpression(whenTrue)) {
        tsExpr = whenTrue;
      } else if (ts51.isCallExpression(whenTrue) && ts51.isPropertyAccessExpression(whenTrue.expression)) {
  ...
  getLiteralCompletionLocation(expr) {
    ...
    if (expr instanceof TmplAstTextAttribute) {
      ...
      if (strNode !== null && ts51.isStringLiteral(strNode.expression)) {
        tsExpr = strNode.expression;
      }
    } else {
      tsExpr = findFirstMatchingNode(this.tcb, {
        filter: (n2) => ts51.isStringLiteral(n2) || ts51.isNumericLiteral(n2),
        withSpan: expr.sourceSpan
      });
    }
    if (tsExpr === null) {
    ...
    if (ts51.isStringLiteral(tsExpr)) {
